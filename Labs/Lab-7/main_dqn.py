...
class Agent_DQN():
    def __init__(self, env, seed = None,
                 ...
                 epsScheduler=Utils_replay.LinearScheduleEpsilon(start_eps=1.0, final_eps=0),  # How to update epsilon ?
        ...
        self.method = method
        self.evaluationMode = False if TRAINING_MODE is True else False
    ...
    def get_action(self, states, epsilon):
        ...
        if self.evaluationMode is False and (self.total_steps <= self.pre_train_steps or \
            ...
            actions = np.argmax(predict_q, axis=1).astype(np.int32)
    ...
    def run_episode(self) -> Tuple[float, float, float, tf.Tensor]:
        ...
        for episode_step in tf.range(self.max_steps_per_episode):
            ...
            epsilon = 0.0 if self.evaluationMode is True else self.epsScheduler.getValue(step=self.total_steps) # Note that we use the total steps in this entire training process, not the local episode step
            ...
            if self.evaluationMode is False:
            ...
            if self.evaluationMode is False and (self.total_steps > self.pre_train_steps and len(self.replayBuffer) >= self.batch_size)\
            ...
            if self.evaluationMode is False and self.method > MethodToUse.DQN_BASE and self.total_steps % self.steps_until_sync == 0:
    ...
    def train(self, max_episodes,
        ...
        EVALUATION_FREQUENCY = 500 # At how many train steps we do an evaluation step ? This is needed to avoid random policies when evaluating
        ...
        with tqdm.trange(self.max_episodes) as t:
            for episode in t:
                ...
                if episode % EVALUATION_FREQUENCY == 0:
                    ...
                    numEpisodesToEvaluate = 100
                    totalEvalReward = 0.0
                    for epEval in range(numEpisodesToEvaluate):
                        reward, loss, epsilonUsed, gradsMagnitude = self.run_episode()
                        assert epsilonUsed == 0.0
                        totalEvalReward += reward
                    avgEvalReward = totalEvalReward / numEpisodesToEvaluate

                    avgEvalReward = avgEvalReward.numpy()
                    print(f"Evaluation results, avg reward in {numEpisodesToEvaluate} evaluated episodes is {avgEvalReward}")
                    if avgEvalReward > reward_threshold:
                        break # Solved !

                    # Activate back
                    self.evaluationMode = False if TRAINING_MODE else False
...
if __name__ == "__main__":
    ...
    agent = Agent_DQN(env, seed=seed, gamma=0.99, batch_size=64, lr=0.007,
                           steps_until_sync=200, choose_action_frequency=1,
                           train_frequency=1, replayBufferSize=32000, pre_train_steps=100,
                           epsScheduler=Utils_replay.LinearScheduleEpsilon(start_eps=1, final_eps=0.01,
